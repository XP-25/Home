...
io.on('connection', (socket) => {
  console.log(`New Socket.IO connection from: ${socket.id}`); // Added log for new connections
  ...
  socket.on('createRoom', ({ name, deviceType }) => {
    ...
    console.log(`Room created: ${roomCode} by player ${safeName}`); // Added log for room creation
  });
  ...
  socket.on('joinRoom', ({ name, roomCode, deviceType }) => {
    ...
    console.log(`Player ${safeName} joined room: ${safeRoomCode}`); // Added log for joining room
  });
  ...
  socket.on('startGame', ({ roomCode }) => {
    ...
    console.log(`Game started in room: ${safeRoomCode}`); // Added log for starting game
  });
  ...
  socket.on('findOpponent', ({ name, deviceType }) => {
    ...
    if (waitingPlayers.length > 0) {
      ...
      console.log(`Match found: Player ${safeName} matched with ${opponent.name} in room ${roomCode}`); // Added log for matchmaking success
      ...
      const timeout = setTimeout(() => {
        ...
        if (playerIndex !== -1) {
          ...
          console.log(`No opponent found for player ${safeName} after timeout`); // Added log for matchmaking timeout
        }
      }, 10000); // 10 seconds
      waitingPlayers.push({ id: socket.id, name: safeName, deviceType: safeDeviceType, timeout });
      console.log(`Player ${safeName} added to waiting queue`); // Added log for adding to queue
    }
  });
  ...
  socket.on('makeMove', ({ roomCode, piece, row, col }) => {
    ...
    if (!validPiece || validRow === null || validCol === null) {
      ...
      console.log(`Invalid move attempted in room ${safeRoomCode}: piece=${piece}, row=${row}, col=${col}`); // Added log for invalid move
      return;
    }
    // Proceed with broadcasting the validated move
    socket.to(safeRoomCode).emit('move', { piece: validPiece, row: validRow, col: validCol });
    console.log(`Move made in room ${safeRoomCode}: piece=${validPiece}, row=${validRow}, col=${validCol}`); // Added log for valid move
  });
  ...
  socket.on('resetGame', ({ roomCode }) => {
    ...
    console.log(`Game reset in room: ${safeRoomCode}`); // Added log for game reset
  });

  // Disconnect
  socket.on('disconnect', () => {
    console.log(`Socket disconnected: ${socket.id}`); // Added log for disconnections
    ...
    if (playerIndex !== -1) {
      ...
      waitingPlayers.splice(playerIndex, 1);
      console.log(`Disconnected player removed from waiting queue: ${socket.id}`); // Added log for queue removal
    }
    // Remove from rooms
    for (const code of Object.keys(rooms)) {
      ...
      if (before === 2 && r.players.length === 1) {
        ...
        r.started = false;
        console.log(`Opponent left in room ${code}, notifying remaining player`); // Added log for opponent leaving
      }
      if (r.players.length === 0) {
        delete rooms[code];
        console.log(`Room ${code} deleted as it is now empty`); // Added log for room deletion
      }
    }
  });
});
