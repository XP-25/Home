...
const io = new Server(server, {
  cors: {
    origin: ["https://raisexp.games", "https://raisexp.games/", "http://localhost:3000"], // Allow your frontend and local dev
    methods: ["GET", "POST"],
    credentials: true
  }
});
...
// ---- Socket.IO ----
io.on('connection', (socket) => {
  console.log('New Socket.IO connection from:', socket.id); // Added for debugging
  // Create a private room
  socket.on('createRoom', ({ name, deviceType }) => {
    const safeName = sanitizeName(name);
    const safeDeviceType = sanitizeDeviceType(deviceType);
    ...
    socket.join(roomCode);
    socket.emit('roomCreated', { roomCode });
    console.log('Room created:', roomCode); // Added for debugging
  });
  // Join a private room
  socket.on('joinRoom', ({ name, roomCode, deviceType }) => {
    const safeName = sanitizeName(name);
    ...
    socket.emit('joinedRoom', { success: true, roomCode: safeRoomCode });
    io.to(room.players[0].id).emit('playerJoined', { name: safeName });
    console.log('Player joined room:', safeRoomCode); // Added for debugging
  });
  // Start game (by creator)
  socket.on('startGame', ({ roomCode }) => {
    const safeRoomCode = sanitizeRoomCode(roomCode);
    const room = rooms[safeRoomCode];
    if (!room || room.players.length !== 2) return;
    if (room.players.id !== socket.id) return;
    if (room.started) return;
    room.nboard = determineGridSize(room.players.deviceType, room.players[1].deviceType);
    room.started = true;
    emitStartGameToBoth(safeRoomCode);
    console.log('Game started in room:', safeRoomCode); // Added for debugging
  });
  // Matchmaking (desktop ↔ mobile supported)
  socket.on('findOpponent', ({ name, deviceType }) => {
    const safeName = sanitizeName(name);
